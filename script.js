...
function handleExplodeAction(row,col,game,cellEl,tableEl){
    ...
    panelEl.innerHTML=` <h3>啊哦，再来一次吧，${game.seconds}秒结束战斗</h3>`;
...
function handleWin(row,col,game,cellEl,tableEl){
    ...
    gameEl.append(panelEl);
    panelEl.innerHTML=` <h3>恭喜，very nice！你在${game.seconds}秒内完成比赛</h3>`;
...
function renderWelcome(){
    ...
    <button id='level1'>中等</button>
    <button id='advance'>困难</button>
    </div>
    `;
    let buttonEl=gameEl.querySelector('button#level0');
    buttonEl.onclick=()=>{    
        let game = new Game(8, 8, 0.05);
    game.shuffle();
    renderTable(game);}

    buttonEl=gameEl.querySelector('button#level1');
    buttonEl.onclick=()=>{    
        let game = new Game(12, 18, 0.1);
    game.shuffle();
    renderTable(game);}
